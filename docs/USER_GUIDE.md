...
**Version 0.5 “Chronicles” · Windows 10/11 (64‑bit)**
...
- **`bibliosense_0.5.0_x64-setup.exe`** — standard installer; adds a Start‑menu
  shortcut. Best for most users.
- **`bibliosense_0.5.0_x64_en-US.msi`** — for IT‑managed machines.
...
### (Optional) Turn on AI
AI features — narrative report writing, screening assistance, the in‑app
assistant — are optional and use **your own** provider key.
...
2. Choose a provider (OpenAI, Anthropic, or **Mistral** — Mistral has a capable
   free tier), paste your API key, and **Test**.
...
1. Choose **Research Pilot** and type your research question (or a rough topic —
   it can suggest sharper questions).
2. **PubMed** runs live and credential‑free. For **Scopus**, add your Scopus API
   ...
   you have it); Scopus then runs live too.
...
2. **Identification** — bring in your search results (or carry them over from a
   bibliometric analysis / Research Pilot consolidation).
...
4. **Retrieval** — fetch open‑access full texts automatically where available.
5. **Themes** — get suggested themes and organise included studies.
6. **Export** — a publishable **PRISMA flow diagram** (SVG + PDF + DOCX) and a
   records table, with the full audit trail for reproducibility.
...
| **A database lane won’t run live** | PubMed needs no key; **Scopus needs your Scopus API key** (Settings gear). Some keys only return full records on a subscribing campus network. |
| **AI features greyed out** | Add and **Test** a provider key, then toggle AI on. |
| **Large corpus feels slow** | 8 GB RAM is recommended for >1000 records; analysis is heaviest on first run. |
